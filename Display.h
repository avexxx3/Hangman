...
void setData(int &i, sf::Texture &texture, sf::Text &text, sf::Event &event) {
    if (event.type == sf::Event::KeyReleased && i < 8) {
...
void createData(sf::VideoMode desktop, sf::Texture &texture, sf::Sprite &sprite) {
    ...
    texture.loadFromFile("Hangman\\Base.png"); //Create chalkboard as a texture
...
void displayData(sf::RenderWindow& window, sf::Sprite sprite, sf::Text data, sf::Text word, sf::Text categ, int i, bool end, bool idk) {
