...
void scalePositionButtons() {
	hangmanSprite.scale(desktop.width * 1.0 / hangmanSprite.getGlobalBounds().width, desktop.width * 1.0 / hangmanSprite.getGlobalBounds().width);
...
void printCredits() {
	while (-(credits.getPosition().y) < credits.getLocalBounds().height + 50 || !rtrnSprite.getGlobalBounds().contains(static_cast<sf::Vector2f>(pos))) {
		if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
			pos = sf::Mouse::getPosition(window);
		window.clear();
...
void setText() {
	...
	loseObject.setCharacterSize(70);
	loseObject.setString("You lose!");
	loseObject.setPosition((desktop.width - winObject.getGlobalBounds().width) / 2, 20);
}

void endScreen() {
	while (!sf::Keyboard::isKeyPressed(sf::Keyboard::Escape) && !rtrnSprite.getGlobalBounds().contains(static_cast<sf::Vector2f>(pos))) {
		if (sf::Mouse::isButtonPressed(sf::Mouse::Left))
			pos = sf::Mouse::getPosition(window);
		window.clear();
		window.draw(bgSprite);
